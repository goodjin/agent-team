...
export class ChatUI {
  ...
  constructor(options: ChatUIOptions = {}) {
    this.inputPrompt = options.inputPrompt || 'You: ';
  ...
  private buildOutputLines(width: number): string[] {
    ...
    for (const role of this.order) {
      ...
      lines.push(header);
      const contentLines = this.wrapText(block.content, width);
  ...
  private buildInputLine(cols: number): { inputLine: string; cursorCol: number } {
    ...
    const full = this.inputPrompt + this.inputBuffer;
    const cursorInFull = this.inputPrompt.length + this.cursorIndex;
    let start = 0;
    ...
    const visible = full.slice(start, start + innerWidth);
    const cursorCol = 1 + (cursorInFull - start);
    const line = '|' + visible.padEnd(innerWidth, ' ') + '|';
